...
floorPlan -site CORE -s 2100 2100 20.0 20.0 20.0 20.0 -fplanOrigin llcorner
fit
# 2. Place memories
# data_mem bank 0&1
...
for {set i 1} {$i < [llength $DMEMNORTH]} {incr i} {
 ...
 create_relative_floorplan -place [lindex $DMEMNORTH $i]  -orient R0 -ref_type object -ref [lindex $DMEMNORTH $prev] -bbox both -horizontal_edge_separate {1 0 1} -vertical_edge_separate {2 40 0}
}
fit
...
for {set i 1} {$i < [llength $DMEMEAST]} {incr i} {
 ...
 create_relative_floorplan -place [lindex $DMEMEAST $i]  -orient R270 -ref_type object -ref [lindex $DMEMEAST $prev] -bbox both -horizontal_edge_separate {3 -40 1} -vertical_edge_separate {0 0 0}
}
fit
...
for {set i 1} {$i < [llength $IMEMSOUTH]} {incr i} {
 ...
 create_relative_floorplan -place [lindex $IMEMSOUTH $i]  -orient R180 -ref_type object -ref [lindex $IMEMSOUTH $prev] -bbox both -horizontal_edge_separate {3 0 3} -vertical_edge_separate {0 -40 2}
}
